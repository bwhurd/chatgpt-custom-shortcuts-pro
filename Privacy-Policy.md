...
- If you use Cloud Sync, your backup stays in a hidden app data folder in your Google Drive. We do not see it.
- Sign in uses a short lived Google token. A small helper service swaps a one time Google code for that token and does not store tokens or settings.
...
- All Drive requests go directly from your browser to Google over HTTPS. The sign in helper exchanges the Google code for a token and does not store tokens or settings.
...
- We do not collect browsing history, page content, keystrokes, or analytics. Content scripts run only on listed sites to provide shortcuts and do not transmit page content.
