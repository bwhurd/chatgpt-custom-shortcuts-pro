...
#   & "C:\Users\bwhur\Dropbox\CGCSP-Github\Setup-And-Clean-CSSv2.ps1" -Run
[CmdletBinding()]
param(
...
Write-Host "== CSS cleanup setup (simple) =="
...
# Project base next to your CSS (avoid dot-prefixed names)
...
$deps = @('postcss','cssnano','purgecss','prettier')
$toInstall = @()
foreach ($d in $deps) {
...
# safelist.txt (optional; for dynamic classes you know you need)
...
# Examples you might keep if JS toggles them:
#active
#open
#show
#/^is-/
...
# clean-css.mjs (simple: purge against a single rendered HTML; combine/flatten; beautify)
...
async function main(){
  ...
  // Purge against your single rendered HTML
  ...
  // Combine/flatten & dedupe. Keep file order broadly intact; only merges may move things locally.
  const postcssResult = await postcss([
    cssnano({
      // default merges & cleans; no explicit global re-sorting of rules
      preset: ["default", {
        // keep rule order broadly intact; mergeRules allowed so duplicates are coalesced
        mergeRules: true
        // we are NOT enabling cssDeclarationSorter here; property order stays as-is
      }]
    })
  ...
  const prettierOptions = await prettier.resolveConfig(process.cwd()).catch(() => null);
  const formattedCss = await prettier.format(optimizedCss, { ...(prettierOptions || {}), parser: "css" });
  const formattedBytes = Buffer.byteLength(formattedCss);
  ...
  const report = [
    ...
    `Final size:    ${toKb(formattedBytes)} (${formattedBytes} bytes)`,
    `Removed selectors count (purge phase): ${rejected.length}`,
    ``,
    `Notes:`,
    `- mergeRules enabled to combine duplicate selectors.`,
    `- No global rule sorting; only local movement from merges may occur.`,
    `- Prettier formats the final CSS for readability.`,
    ``,
  ...
  await fs.writeFile(finalOut, formattedCss, "utf8");
...
Write-Host "Outputs next to your CSS: *.purged.css, *.scrubbed.css, *.scrubbed.report.txt"