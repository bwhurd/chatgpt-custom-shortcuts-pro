...
//≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡
//≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡
// Sync Settings to Google IIFE
//≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡

(() => {
  const cloudAuth = window.CloudAuth;
  const cloudStorage = window.CloudStorage;
  ...
  const btnLoginMenu = document.getElementById('btnGoogleLoginMenu');
  const btnSync = document.getElementById('btnSyncToCloud');
  const btnRestore = document.getElementById('btnRestoreFromCloud');
  const btnLogout = document.getElementById('btnLogout');
  const statusEl = document.getElementById('syncStatus');
  const avatarBtn = document.getElementById('cloudSyncUserBtn');
  const avatarImg = document.getElementById('cloudSyncUserAvatar');
  const helpIcon = document.getElementById('cloudSyncHelpIcon');
  const menu = document.getElementById('cloudSyncMenu');
  const menuOut = document.getElementById('cloudSyncMenuLoggedOut');
  const menuIn = document.getElementById('cloudSyncMenuLoggedIn');
  ...
  // Normalize a Google avatar URL without breaking sizing.
  function normalizeGoogleAvatarUrl(rawUrl, size = 64) {
    if (!rawUrl) return '';
    try {
      const u = new URL(rawUrl);
      const host = u.hostname || '';
      const isGoogleImg = host.endsWith('googleusercontent.com') || host.endsWith('ggpht.com');

      if (isGoogleImg) {
        // Prefer official knobs: either "?sz=" or path suffix "=s{n}-c"
        if (u.searchParams.has('sz')) {
          u.searchParams.set('sz', String(size));
        } else if (/=s\d+(-[a-z]+)*$/i.test(u.pathname)) {
          u.pathname = u.pathname.replace(/=s\d+(-[a-z]+)*$/i, `=s${size}-c`);
        } else {
          u.searchParams.set('sz', String(size));
        }
      }
      // Always HTTPS, always cache-bust lightly to beat stale proxies.
      u.protocol = 'https:';
      u.searchParams.set('cb', Date.now().toString());
      return u.toString();
    } catch {
      // If it's not an absolute URL, fall back to the raw string.
      return rawUrl;
    }
  }

  // Load avatar and gracefully fall back to help icon on failure.
  function loadAvatar(rawUrl, attempt = 0, size = 64) {
    if (!avatarImg || !rawUrl) return;
    const finalUrl = normalizeGoogleAvatarUrl(rawUrl, size);

    // Start hidden to avoid showing a broken frame.
    helpIcon && (helpIcon.style.display = 'none');
    ...
    avatarImg.onerror = () => {
      if (attempt < 2) {
        const nextSize = attempt === 0 ? 96 : 48; // try different cached variants
        setTimeout(() => loadAvatar(rawUrl, attempt + 1, nextSize), 250 * (attempt + 1));
      } else {
        avatarImg.style.display = 'none';
        helpIcon && (helpIcon.style.display = 'inline-flex');
      }
    };

    if (avatarImg.src !== finalUrl) avatarImg.src = finalUrl;
  }

  // Keep all the places the profile picture might live.
  ...
  function uiLoggedOut() {
    ...
    if (helpIcon) helpIcon.style.display = 'inline-flex';
    if (menuOut) menuOut.style.display = 'block';
    if (menuIn) menuIn.style.display = 'none';
    if (menu) menu.classList.remove('open', 'open-up', 'align-left');
    setStatus('Not linked');
  }

  function uiLoggedIn(profile) {
    if (signinRow) signinRow.style.display = 'none';
    if (syncRow) syncRow.style.display = 'flex';
    if (menuOut) menuOut.style.display = 'none';
    if (menuIn) menuIn.style.display = 'block';
    const email = profile?.email || profile?.userinfo?.email || '';
    setStatus(email ? `Linked to ${email}` : 'Linked');
    // Show avatar once real image loads; keep help icon hidden thereafter.
    const avatarUrl = pickAvatarUrl(profile);
    if (avatarUrl) loadAvatar(avatarUrl);
  }

  // Back-compat alias so other calls (if any) still work.
  function loadAvatarWithRetry(url, attempt = 0) {
    return loadAvatar(url, attempt);
  }

  async function refreshAvatarFromAuth() {
    try {
      if (!cloudAuth?.getSavedAuth) return;
      const { profile } = await cloudAuth.getSavedAuth();
      if (profile) uiLoggedIn(profile);
    } catch (e) {
      console.warn('[cloud-sync] refreshAvatarFromAuth failed', e);
    }
  }

  async function handleLoginClick() {
    // even if CloudAuth missing, at least show a status
    if (!cloudAuth?.googleLogin) {
      setStatus('Sign-in not available', 'error');
      return;
    }
    try {
      setStatus('Signing in…');
      const { profile } = await cloudAuth.googleLogin();
      uiLoggedIn(profile);
    } catch (err) {
      const msg = String(err?.message || err);
      setStatus(msg.includes('Canceled') ? 'Sign-in canceled.' : 'Sign-in failed', 'error');
    }
  }

  async function handleLogoutClick() {
    try {
      await cloudAuth?.googleLogout?.();
    } catch (e) {}
    uiLoggedOut();
  }
  ...
  btnSync?.addEventListener('click', async () => {
    ...
    try {
      setStatus('Syncing…');
      const local = await cloudStorage.loadLocalSettings();
      await cloudStorage.saveSyncedSettings(local);
  ...
  btnRestore?.addEventListener('click', async () => {
    ...
    try {
      setStatus('Restoring…');
      const local = await cloudStorage.loadLocalSettings();
      const remote = await cloudStorage.loadSyncedSettings();
      const merged = cloudStorage.mergeSettings(local, remote);
      await cloudStorage.saveLocalSettings(merged);
  ...
  if (avatarBtn && menu) {
    const closeMenu = () => {
      menu.classList.remove('open', 'open-up', 'align-left');
    ...
    avatarBtn.addEventListener('click', async (e) => {
      ...
      if (willOpen) {
        requestAnimationFrame(() => {
          menu.classList.remove('open-up', 'align-left');
          const r = menu.getBoundingClientRect();
          const vw = innerWidth,
            vh = innerHeight;
          if (r.bottom > vh - 4) menu.classList.add('open-up');
          if (r.right > vw - 4) menu.classList.add('align-left');
        });
        // refresh avatar info when opening, in case it arrived late
        await refreshAvatarFromAuth();
      }
    });

    // Interactions inside menu shouldn't close it immediately
  ...
  (async () => {
    // init: try real auth first, else fallback to "logged out" UI
    if (cloudAuth?.getSavedAuth) {
      try {
        const { profile } = await cloudAuth.getSavedAuth();
        profile ? uiLoggedIn(profile) : uiLoggedOut();
      } catch (e) {
        uiLoggedOut();
      }
    } else {
...
(() => {
  const busy = (btn, isBusy) => {
    ...
    if (!icon) return;
    if (isBusy) {
      btn.dataset.prevIcon = icon.textContent;
      ...
      if (sp) {
        ...
        i.className = 'msr';
        i.textContent =
          btn.dataset.prevIcon || btn.getAttribute('data-default-icon') || 'check_circle';
        sp.replaceWith(i);
      }
    }
  };
