...
- Keep the repo's current `plans/` and `specs/` folder names unless a separate project decision changes casing later.
...
- [x] Update `AGENTS.md`, `PROJECT_SPEC.md`, and the spec docs so folder roles, plan protocol, `_temp-files/`, and shared-tool paths match the cleaned structure.
