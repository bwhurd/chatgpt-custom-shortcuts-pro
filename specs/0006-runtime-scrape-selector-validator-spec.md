...
- codebox live probes must not mutate the fixed fixture conversation; `shortcutKeyToggleCodeboxWrap` should create a blank conversation, send `give me a 300 word story about sword fighting rats in space in a single codebox`, wait at least 15 seconds, and verify an assistant codebox exists before toggling wrap. `shortcutKeyCopyAllCodeBlocks` should reuse that conversation when available or create it when run alone, send another similar single-codebox prompt, wait at least 15 seconds, verify at least two assistant codeboxes exist, then assert the clipboard contains the multi-codeblock separator.
