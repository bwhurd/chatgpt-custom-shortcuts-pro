...
(function () {
    chrome.storage.sync.get(
        ...
        ({ fadeSlimSidebarEnabled: enabled }) => {
            ...
            // Returns a promise for the latest opacity value in storage
            ...
            (async function main() {
                ...
                let idleOpacity = await getIdleOpacity();
                let fadeT = null;
                let isHovered = false;

                function fadeToIdle() {
                    if (!isHovered) {
                        bar.style.opacity = idleOpacity.toString();
                    }
                }
                ...
                chrome.storage.onChanged.addListener((changes, area) => {
                    ...
                    if ('popupSlimSidebarOpacityValue' in changes) {
                        idleOpacity = typeof changes.popupSlimSidebarOpacityValue.newValue === 'number'
